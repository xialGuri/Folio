...
#### Version 1.1
1. 회원님을 위한 추천
- (미해결): 로그인 전에 회원님을 위한 추천 부분 사라짐 or 모양은 남기고 안에 Text를 바꿔줌 
...
-(미해결): 글 작성, 글 수정 모달 부분에서 아이콘 옆에 사용자의 이름 노출 필요
### Back
-(미해결): 작성된 글을 저장 할 수 있는 Back 필요
-(미해결): 수정 모달 창에서 삭제 버튼을 클릭시 Back에 있는 데이터 삭제 밑, Main 화면 게시글 부분 삭제 처리
